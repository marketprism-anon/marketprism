...
-   [Researchlabab](./platform/ResearchlabAB.md):
...
to be run on localhost or Researchlabab.
...
- `Researchlabab` - the system will distribute the `Hosts` nodes over the
 available servers, but not over more than `Servers`.
 This allows for running simulations that are smaller than your whole Researchlabab experiment without having to modify and restart the
  experiment.
- `Mininet` - as in `Researchlabab`, the `Hosts` nodes will be distributed over
...
However, there will not be more nodes available than the ones in the prepared
 `Roster`.
Some restrictions apply when you're using the `Researchlabab` simulation: 
...
-   `PreScript` - a shell-script that is run _before_ the simulation is started
    ...
    [localhost,mininet,Researchlabab]
