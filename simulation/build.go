...
// todoCoder: change it to ghada lab later
...
//var platformDst = "ResearchLabab"
//var platformDst string //= "ResearchLabab"
...
func init() {
	...
	flag.IntVar(&machines, "machines", machines, "Number of machines on ResearchLabab")
	...
	flag.StringVar(&platformDst, "platform", platformDst, "platform to deploy to [localhost,mininet,ResearchLabab]")
...
func startBuild(platformDstDefault string) {
	...
	for _, simulation := range simulations {
		...
		/* : instead of reading these config variables from the toml connfig file, we set an initialized
		   ...
		   e.g.: go test -platform=ResearchLabab -MCRoundDuration=10 -timeout 300000s -run ^TestSimulation$
		*/
		...
		if clean {
			...
			// set timeout for the experiment from the config file
...
func RunTest(deployP platform.Platform, rc *platform.RunConfig) error {
	...
	go func() {
		...
		} else if platformDst == "ResearchLabab" {
		...
		} else if platformDst == "ResearchLab"{
			done <- nil
		}
	}()
