...
func (d *ResearchLab) loadAndCheckResearchLabVars() {
	...
	if d.Host == "" {
		d.Host = readString("Please enter the hostname of ResearchLab", "ResearchLab-Url")
	}
	...
	if d.MonitorAddress == "" {
		d.MonitorAddress = readString("Please enter the Monitor address (where clients will connect)", "ResearchLab-Url:22")
	}
	if d.ProxyAddress == "" {
