...
type ChainBoost struct {
	...
	*onet.TreeNodeInstance
	...
	RtLSideChainNewRoundChanChain2 chan RtLSideChainNewRoundChanChain2
	LtRSideChainNewRoundChanChain2 chan LtRSideChainNewRoundChanChain2

	RtLSideChainNewRoundChanChain3 chan RtLSideChainNewRoundChanChain3
	LtRSideChainNewRoundChanChain3 chan LtRSideChainNewRoundChanChain3

	RtLSideChainNewRoundChanChain4 chan RtLSideChainNewRoundChanChain4
	LtRSideChainNewRoundChanChain4 chan LtRSideChainNewRoundChanChain4

	RtLSideChainNewRoundChanChain5 chan RtLSideChainNewRoundChanChain5
	LtRSideChainNewRoundChanChain5 chan LtRSideChainNewRoundChanChain5
	...
	CommitteeNodesTreeNodeID2 []onet.TreeNodeID
	CommitteeNodesTreeNodeID3 []onet.TreeNodeID
	CommitteeNodesTreeNodeID4 []onet.TreeNodeID
	CommitteeNodesTreeNodeID5 []onet.TreeNodeID
	NextSideChainLeader1      onet.TreeNodeID
	NextSideChainLeader2      onet.TreeNodeID
	NextSideChainLeader3      onet.TreeNodeID
	NextSideChainLeader4      onet.TreeNodeID
	NextSideChainLeader5      onet.TreeNodeID
	CountNodes                int
	FaultyContractsRate       float64
}
...
func (bz *ChainBoost) Dispatch() error {
	...
	for running {
		...
		case msg := <-bz.RtLSideChainNewRoundChanChain2:
			go bz.SideChainLeaderPreNewRound("marketmatch", msg)

		case msg := <-bz.RtLSideChainNewRoundChanChain3:
			go bz.SideChainLeaderPreNewRound("dispute", msg)

		case msg := <-bz.RtLSideChainNewRoundChanChain4:
			go bz.SideChainLeaderPreNewRound("por2", msg)

		case msg := <-bz.RtLSideChainNewRoundChanChain5:
			go bz.SideChainLeaderPreNewRound("por3", msg)
		// -----------------------------------------------------------------------------
		...
		case msg := <-bz.LtRSideChainNewRoundChanChain2:
			go func() {
				//time.Sleep(time.Duration(bz.SCRoundDuration) * time.Second)
				bz.SideChainRootPostNewRound("marketmatch", msg)
			}()

		case msg := <-bz.LtRSideChainNewRoundChanChain3:
			go func() {
				//time.Sleep(time.Duration(bz.SCRoundDuration) * time.Second)
				bz.SideChainRootPostNewRound("dispute", msg)
			}()

		case msg := <-bz.LtRSideChainNewRoundChanChain4:
			go func() {
				//time.Sleep(time.Duration(bz.SCRoundDuration) * time.Second)
				bz.SideChainRootPostNewRound("por2", msg)
			}()

		case msg := <-bz.LtRSideChainNewRoundChanChain5:
			go func() {
				//time.Sleep(time.Duration(bz.SCRoundDuration) * time.Second)
				bz.SideChainRootPostNewRound("por3", msg)
			}()
		...
		case sig := <-bz.BlsCosi.Load().FinalSignature:
			...
			if str == "por" {
				...
				err := bz.SendTo(bz.Root(), &LtRSideChainNewRoundChain3{
					NewRound:      true,
			...
			} else if str == "marketmatch" {
				err := bz.SendTo(bz.Root(), &LtRSideChainNewRoundChain2{
					NewRound:      true,
				...
				err := bz.SendTo(bz.Root(), &LtRSideChainNewRoundChain4{
					NewRound:      true,
				...
				err := bz.SendTo(bz.Root(), &LtRSideChainNewRoundChain5{
					NewRound:      true,
					SCRoundNumber: int(bz.SCRoundNumber["por3"].Load()),
					SCSig:         sig,
				})
				if err != nil {
					return xerrors.New("can't send new round msg to root" + err.Error())
