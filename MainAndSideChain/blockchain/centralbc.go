...
func InitializeMainChainBC(exconf ExperimentConfig) {
	...
	for j := 0; j < exconf.ShardCount; j++ {
		var err error
		shardname := fmt.Sprintf("shard%d", j)
		err = InitalizeMainChainDbTables(shardname)
		if err != nil {
			panic(err)
		}
		...
		for i, _ := range FileSizeRow {
			bc := flip(0.7)
			err = InitialInsertValuesIntoMarketMatchingTable(shardname, int(FileSizeRow[i].(float64)), int(ServAgrDurationRow[i].(float64)), 0, i+1, bc, bc, int(NegotiationDurationRow[i].(float64)))
			if err != nil {
				panic(err)
			}
		}
		err = UpdateActiveFaultyContracts(shardname, exconf.FaultyContractsRate)

		SimulationSeedAsStr := strconv.Itoa(exconf.SimulationSeed)
		err = InsertIntoMainChainRoundTable(shardname, 0, SimulationSeedAsStr, 0, "", 0, 0, 0, 0, 0, time.Now(), 0, 0, 0, false, false, false)
		if err != nil {
			panic(err)
		}
		...
		hash := sha.Sum(nil)
		err = InsertIntoMainChainRoundTable(shardname, 1, hex.EncodeToString(hash), 0, "", 0, 0, 0, 0, 0, time.Now(), 0, 0, 0, false, false, false)
	}
	logMsg := fmt.Sprintf(
