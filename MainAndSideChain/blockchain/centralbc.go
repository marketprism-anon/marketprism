...
func InitializeMainChainBC(exconf ExperimentConfig) {
	...
	for i, _ := range FileSizeRow {
		bc := flip(0.7)
		err = InitialInsertValuesIntoMarketMatchingTable(int(FileSizeRow[i].(float64)), int(ServAgrDurationRow[i].(float64)), 0, i+1, bc, false, int(NegotiationDurationRow[i].(float64)))
		if err != nil {
			panic(err)
		}
	}
