...
func InsertIntoMainChainMarketMatchingTable(serverInfo string,
	...
	_, err = mainchainDb.Exec(`INSERT INTO MarketMatching(ServerInfo, FileSize, ServAgrDuration,
                                StartedMcRoundNumber, ServAgrId, Published, TxPayable)
                                 VALUES (?, ?, ?, ?, ?, ?, ?)`,
		serverInfo, fileSize, serverAgrDuration, startedMcRoundNumber,
		serverAgrId, published, TxPayable)
	return err
}
...
func InitialInsertValuesIntoMarketMatchingTable(Filesize int,
	...
	sqlStmt := "INSERT INTO MarketMatching (FileSize, ServAgrDuration, StartedMcRoundNumber, ServAgrId, Published, TxPayable, NegotiationDuration) VALUES (?, ?, ?, ?, ?, ?, ?)"
	_, err = mainchainDb.Exec(sqlStmt, Filesize, ServAgrDuration, StartedMcRoundNumber, ServAgrId, Published, TxPayable, NegotiationDuration)
	return err
}
...
func MainChainSetTxPayable(rowId int, value bool) error {
	//mcMutex.Lock()
	//defer mcMutex.Unlock()
	var err error
	if mainchainDb == nil {
	...
	_, err = mainchainDb.Exec("UPDATE MarketMatching Set TxPayable=? Where rowid = ?", value, rowId)
	if err != nil {
		return err
	}
	return nil
