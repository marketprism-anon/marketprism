...
func (bz *ChainBoost) StartMainChainProtocol() {
	...
	for {
		name := fmt.Sprintf("shard%d", bz.CurrentShard.Load())
		bz.updateBCPowerRound(name, bz.TreeNode().Name(), true)
		if bz.CurrentShard.Load() == 0 {
			bz.updateMainChainBCTransactionQueueCollect(name)
		}
		bz.updateMainChainBCTransactionQueueTake(name)
		if bz.SimState == 2 {
			go bz.StartSideChainProtocol()
		}
		if bz.CurrentShard.Load() == int64(bz.ShardsCount)-1 {
			bz.readBCAndSendtoOthers(name)
		}
		bz.CurrentShard.Store(bz.CurrentShard.Inc() % int64(bz.ShardsCount))
		if bz.CurrentShard.Load() == 0 {
			break
		}
	}
...
func (bz *ChainBoost) RootPreNewRound(msg MainChainNewLeaderChan) {
	...
	if msg.LeaderTreeNodeID == bz.TreeNode().ID && bz.MCRoundNumber.Load() != 1 && bz.MCLeader.HasLeader && int64(msg.MCRoundNumber) == bz.MCRoundNumber.Load() {
		...
		for {
			...
			eachFunctionTakenTime = time.Now()
			bz.updateBCPowerRound(name, bz.Tree().Search(msg.LeaderTreeNodeID).Name(), true)
			if bz.CurrentShard.Load() == 0 {
