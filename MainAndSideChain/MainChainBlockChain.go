...
func (bz *ChainBoost) updateMainChainBCTransactionQueueCollect() {
	...
	var PorTxSize, _, PayTxSize, StoragePayTxSize, _ uint32
	PorTxSize, _, PayTxSize, StoragePayTxSize, _, _ = blockchain.TransactionMeasurement(bz.SectorNumber, bz.SimulationSeed)

	var numOfPoRTxsMC, numOfPoRTxsSC, numOfServAgrProposeTxs, numOfStoragePaymentTxs, numOfServAgrCommitTxs, numOfRegularPaymentTxs, numDisputes, toActivate int
	...
	for i := range rowsMarketMatching {
		if rowsMarketMatching[i].TxPayable {
			err = blockchain.MainChainSetTxPayable(i+1, false)
			if err != nil {
				panic(err)
			}
		...
		if !rowsMarketMatching[i].Published {
			if bz.MCRoundNumber.Load() == 1 {
				if gt.MMActivationTimes == nil {
					gt.MMActivationTimes = make(map[int]int)
				}
				if _, ok := gt.MMActivationTimes[i+1]; !ok {
					gt.MMActivationTimes[i+1] = rowsMarketMatching[i].NegotiationDuration
					toActivate++
				}
			}

		} else if rowsMarketMatching[i].Published && int(bz.MCRoundNumber.Load())-rowsMarketMatching[i].StartedMcRoundNumber < 0 {
		...
		} else if rowsMarketMatching[i].Published && int(bz.MCRoundNumber.Load())-rowsMarketMatching[i].StartedMcRoundNumber > rowsMarketMatching[i].ServerAgrDuration { //Contract Expired, set published to false; add to inactive set, set to payable

			blockchain.MainChainSetTxPayable(i+1, true)
			blockchain.MainChainSetPublished(i+1, false)
			if gt.MMActivationTimes == nil {
				gt.MMActivationTimes = make(map[int]int)
			}
			if _, ok := gt.MMActivationTimes[i+1]; !ok {
	...
	if bz.PayPercentOfTransactions == 0 || bz.PayPercentOfTransactions > 1 {
		...
		totalNonPayTx := len(mcFirstQueueTxs)
		for _, v := range scFirstQueueTxs {
			totalNonPayTx += len(v)
		}
	...
	log.LLvl1(bz.Name(), " finished collecting new transactions to mainchain queues in mc round number ", bz.MCRoundNumber, "in total:\n ",
		...
		"and ", numOfPoRTxsSC, "numOfPoRTxsSC added to the side chain queue",
		"and ", numDisputes, "Disputes added to the side chain queue",
		"and", toActivate, "Contracts to MarketMatch")
