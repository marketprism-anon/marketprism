...
------------------------------------------------------------------------
*/
func (bz *ChainBoost) finalMainChainBCInitialization(name string) {
	...
	maxFileSize, err := blockchain.GetMaxMainChain(name, "MarketMatching", "FileSize")
	if err != nil {
		log.Fatal(err)
	}
	...
	err = blockchain.AddMoreFieldsIntoTableInMainChain(name, "MarketMatching", "ServerInfo", argvars2...)
	if err != nil {
		log.Fatal(err)
	}
	...
	*/ // Can be obtained from Table using formulas
	// --- power table sheet
	err = blockchain.InitialInsertValuesIntoMainChainPowerTable(name, argvars...)
	if err != nil {
		log.LLvl1("Panic Raised:\n\n")
		panic(err)
	}
...
------------------------------------------------------------------------
*/
func (bz *ChainBoost) readBCAndSendtoOthers(name string) {
	...
	takenTime := time.Now()
	powers, seed := bz.readBCPowersAndSeed(name)
	bz.MCRoundNumber.Inc()
	// ---
...
/* ----------------------------------------------------------------------*/
func (bz *ChainBoost) readBCPowersAndSeed(name string) (minerspowers map[string]int, seed string) {
	var err error
	takenTime := time.Now()
	seed, err = blockchain.MainChainGetLastRoundSeed(name)
	if err != nil {
		log.Fatal(err)
	}

	minerspowers, err = blockchain.MainChainGetPowerTable(name)
	if err != nil {
		log.Fatal(err)
	}
...
func (bz *ChainBoost) UpdateFaultyContracts(name string) {
	err := blockchain.UpdateActiveFaultyContracts(name, bz.FaultyContractsRate)
	if err != nil {
		panic(err)
	}
}

func (bz *ChainBoost) updateBCPowerRound(name string, LeaderName string, leader bool) {
	var seed string
	var err error
	takenTime := time.Now()
	seed, err = blockchain.MainChainGetLastRoundSeed(name)
	...
	err = blockchain.InsertIntoMainChainRoundTable(name, nextRoundNumber, nextSeed, 0, LeaderName, 0, 0, 0, 0, 0, time.Now(), 0, 0, 0, false, false, false)
	if err != nil {
		log.LLvl1("Panic Raised:\n\n")
		panic(err)
	}
	...
	rowsMarketMatching, err := blockchain.MainChainGetMarketMatchingRows(name)
	if err != nil {
		log.LLvl1("Panic Raised:\n\n")
		panic(err)
	}
	...
	err = blockchain.MainChainUpdatePowerTable(name, MinerServers)
...
func (bz *ChainBoost) addNewRound(name string, LeaderName string) {
	seed, err := blockchain.MainChainGetLastRoundSeed(name)
	nextRoundNumber := int(bz.MCRoundNumber.Load() + 1)
	data := fmt.Sprintf("%v", seed)
	sha := sha256.New()
...
func (bz *ChainBoost) updateMainChainBCTransactionQueueCollect(name string) {
	takenTime := time.Now()
	var err error
	...
	var numOfPoRTxsMC, numOfPoRTxsSC, numOfServAgrProposeTxs, numOfStoragePaymentTxs, numOfServAgrCommitTxs, numOfRegularPaymentTxs, numTx, numXsTx int

	// -------------------------------------------------------------------------------
	mcFirstQueueTxs := make(map[string][]blockchain.MainChainFirstQueueEntry)
	scFirstQueueTxs := make(map[string][]blockchain.SideChainFirstQueueEntry)
	mcSecondQueueTxs := make(map[string][]blockchain.MainChainSecondQueueEntry, 0)
	for _, sc := range bz.Sidechains {
		scFirstQueueTxs[sc] = make([]blockchain.SideChainFirstQueueEntry, 0)
	}

	for i := 0; i < bz.ShardsCount; i++ {
		mcFirstQueueTxs[fmt.Sprintf("shard%d", i)] = make([]blockchain.MainChainFirstQueueEntry, 0)
		mcSecondQueueTxs[fmt.Sprintf("shard%d", i)] = make([]blockchain.MainChainSecondQueueEntry, 0)
	}
	...
	for i := range rowsMarketMatching {
		...
		if !rowsMarketMatching[i].Published && !rowsMarketMatching[i].TxIssued {
			if gt.MMActivationTimes[i] != 0 {
				firstRnd := rand.Intn(bz.ShardsCount)
				tx := blockchain.MainChainFirstQueueEntry{Name: "TxServAgrPropose", Size: ServAgrProposeTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
				mcFirstQueueTxs[fmt.Sprintf("shard%d", firstRnd)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", firstRnd)], tx)
				numOfServAgrProposeTxs++
				numTx++
				tx = blockchain.MainChainFirstQueueEntry{Name: "TxServAgrCommit", Size: ServAgrCommitTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
				secondRnd := rand.Intn(bz.ShardsCount)
				mcFirstQueueTxs[fmt.Sprintf("shard%d", secondRnd)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", secondRnd)], tx)
				numOfServAgrCommitTxs++
				numTx++
				if firstRnd != secondRnd {
					tx := blockchain.MainChainFirstQueueEntry{Name: "TxServAgrProposeXS", Size: ServAgrProposeTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
					mcFirstQueueTxs[fmt.Sprintf("shard%d", secondRnd)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", secondRnd)], tx)
					numTx++
					numXsTx++
				}
				...
				tx := blockchain.MainChainFirstQueueEntry{Name: "TxServDeal", Size: FinalCommitSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
				secondRnd := rand.Intn(bz.ShardsCount)
				mcFirstQueueTxs[fmt.Sprintf("shard%d", secondRnd)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", secondRnd)], tx)
				numTx++
				err = blockchain.MainChainSetTxIssued(name, i+1, true)
				if err != nil {
					panic(err)
				}
			...
			if bz.StoragePaymentEpoch != 0 {
				...
				err = blockchain.MainChainSetPublished(name, i+1, false)
				if err != nil {
					panic(err)
				}
				// i start from index 0, serverAgreementID strats from index 2 which is its rownumber
				err = blockchain.MainChainSetTxIssued(name, i+1, false)
				if err != nil {
					panic(err)
				}
				...
				mcFirstQueueTxs[fmt.Sprintf("shard%d", rnd)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", rnd)], tx)
				numTx++
				if rand.Float64() < 0.9 {
					tx := blockchain.MainChainFirstQueueEntry{Name: "TxStoragePaymentXS", Size: PorTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
					mcFirstQueueTxs[fmt.Sprintf("shard%d", rnd)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", rnd)], tx)
					numTx++
					numXsTx++
				}
			...
			if txName == "TxDispute" {
				for i = 0; i < bz.ShardsCount; i++ {
					if i != rnd {
						tx := blockchain.MainChainFirstQueueEntry{Name: "TxDisputeXS", Size: PorTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load()), ServAgrId: i + 1}
						mcFirstQueueTxs[fmt.Sprintf("shard%d", i)] = append(mcFirstQueueTxs[fmt.Sprintf("shard%d", i)], tx)
						numTx++
						numXsTx++
					}
				}
			...
			issuedRoundNumber := (int(bz.SCRoundNumber["por"].Load()-1) % bz.SCRoundPerEpoch)
			...
			scFirstQueueTxs["por"] = append(scFirstQueueTxs["por"], tx)
	...
	for k, v := range mcFirstQueueTxs {
		for i := 0; i < len(v); i += 2000 {
			limit := math.Min(float64(len(v)), float64(i+2000))

			err = blockchain.BulkInsertIntoMainChainFirstQueue(k, v[i:int(limit)])
			if err != nil {
				panic(err)
			}
		}
	...
	for i := 1; i <= numberOfRegPay; i++ {
		...
		mcSecondQueueTxs[fmt.Sprintf("shard%d", rnd)] = append(mcSecondQueueTxs[fmt.Sprintf("shard%d", rnd)], tx)
		numTx++
		if rand.Float64() < 0.9 {
			tx := blockchain.MainChainSecondQueueEntry{Size: PayTxSize, Time: time.Now(), RoundIssued: int(bz.MCRoundNumber.Load())}
			mcSecondQueueTxs[fmt.Sprintf("shard%d", rnd)] = append(mcSecondQueueTxs[fmt.Sprintf("shard%d", rnd)], tx)
	...
	for k, v := range mcSecondQueueTxs {
		for i := 0; i < len(v); i += 2000 {
			limit := math.Min(float64(len(v)), float64(i+2000))
			blockchain.BulkInsertIntoMainChainSecondQueue(k, v[i:int(limit)])
			if err != nil {
				panic(err)
			}
		}
	...
	err = blockchain.InsertXsTs(name, int(bz.MCRoundNumber.Load()), numTx, numXsTx)
	if err != nil {
		panic(err)
	}
}
...
------------------------------------------------------------------------
*/
func (bz *ChainBoost) updateMainChainBCTransactionQueueTake(name string) {
	var err error
	// --- reset
	bz.FirstQueueWait = 0
	...
	index := 0
	lastRowId := 0
	rows, err := blockchain.MainChainGetSecondQueue(name)
	if err != nil {
		panic(err)
	}
	...
	for index = 0; index <= len(rows)-1 && !regPayshareIsFull; index++ {
		...
		if 100*(accumulatedTxSize+int(out.Size)) <= regPayTxShare {
			...
			blockchain.AddToRoundTableBasedOnRoundNumber(name, "RegSpaceFull", true, int(bz.MCRoundNumber.Load()))
			break
		}
		//}
		//}
	}
	blockchain.MainChainDeleteFromSecondQueue(name, lastRowId)
	...
	numberOfSyncTx := 0
	numberOfCrossShardTx := 0
	numberOfDisputeTx := 0
	numberOfServAgrDealTx := 0
	SCPoRTx := 0

	txs, err := blockchain.MainChainPopFromSyncTxQueue(name)
	if err != nil {
		panic(err)
	}
	...
	takenTime = time.Now()
	fqRows, err := blockchain.MainChainGetFirstQueue(name)
	if err != nil {
		panic(err)
	}
	index = 0
	...
	for index = 0; index <= len(fqRows)-1 && !blockIsFull; index++ {
		...
		if accumulatedTxSize+int(out.Size) <= otherTxsShare {
			...
			switch out.Name {
			case "TxServDeal":
				err = blockchain.MainChainSetPublishedAndStartRoundOnServAgrId("shard0", out.ServAgrId, true, int(bz.MCRoundNumber.Load()))
				if err != nil {
					panic(err)
				}
				numberOfServAgrDealTx++
			...
			case "TxStoragePaymentXS":
				numberOfCrossShardTx++
			case "TxServAgrProposeXS":
				numberOfCrossShardTx++
			case "TxDisputeXS":
				numberOfCrossShardTx++
			case "TxDispute":
			...
			lastRowId = out.RowId
			log.Lvl2("1 other tx is taken.", out.Name, index)
		} else {
			blockIsFull = true
			log.Lvl1("Final result MC: \n block is full! ")
			err = blockchain.AddToRoundTableBasedOnRoundNumber(name, "BlockSpaceFull", true, int(bz.MCRoundNumber.Load()))
			if err != nil {
				panic(err)
			}
			break
	...
	err = blockchain.MainChainDeleteFromFirstQueue(name, lastRowId)
	if err != nil {
		panic(err)
	}
	...
	log.Lvl1("In total in mc round number ", bz.MCRoundNumber,
		...
		"\n number of published deal payment transactions is", numberOfServAgrDealTx,
		"\n number of published XSTx payment transactions is", numberOfCrossShardTx,
	)

	TotalNumTxsInBothQueue := numberOfPoRTx + numberOfStoragePayTx + numberOfServAgrProposeTx + numberOfServAgrCommitTx + numberOfRegPayTx + numberOfSyncTx + numberOfCrossShardTx + numberOfServAgrDealTx + numberOfDisputeTx
	TotalNumTxsInFirstQueue := numberOfPoRTx + numberOfStoragePayTx + numberOfServAgrProposeTx + numberOfServAgrCommitTx + numberOfSyncTx + numberOfCrossShardTx + numberOfServAgrDealTx + numberOfDisputeTx
	...
	err = blockchain.AddStatsToRoundTableBasedOnRoundNumber(name, BlockSize, numberOfRegPayTx, numberOfPoRTx, numberOfStoragePayTx, numberOfServAgrProposeTx, numberOfServAgrCommitTx,
	...
	SumBCSize, err := blockchain.GetSumMainChain(name, "RoundTable", "BCSize")
	if err != nil {
		log.LLvl1("Panic Raised:\n\n")
		panic(err)
	}

	SumRegPayTx, err := blockchain.GetSumMainChain(name, "RoundTable", "RegPayTx")
	if err != nil {
		log.LLvl1(err)
	}

	SumPoRTx, err := blockchain.GetSumMainChain(name, "RoundTable", "PoRTx")
	if err != nil {
		log.LLvl1(err)
	}

	SumStorjTx, err := blockchain.GetSumMainChain(name, "RoundTable", "StorjPayTx")
	if err != nil {
		log.LLvl1(err)
	}

	SumCntPropTx, err := blockchain.GetSumMainChain(name, "RoundTable", "CntPropTx")
	if err != nil {
		log.LLvl1(err)
	}

	SumCntCmtTx, err := blockchain.GetSumMainChain(name, "RoundTable", "CntCmtTx")
	if err != nil {
		log.LLvl1(err)
	}

	AvgWaitForOtherTxs, err := blockchain.GetAvgMainChain(name, "RoundTable", "AveWaitOtherTxs")
	if err != nil {
		log.LLvl1(err)
	}

	AvgWaitRegPay, err := blockchain.GetAvgMainChain(name, "RoundTable", "AveWaitRegPay")
	if err != nil {
		log.LLvl1(err)
	}

	err = blockchain.InsertIntoMainChainOverallEvaluationTable(name, int(bz.MCRoundNumber.Load()), SumBCSize, SumRegPayTx, SumPoRTx, SumStorjTx,
...
func (bz *ChainBoost) syncMainChainBCTransactionQueueCollect(name string) (blocksize int) {
	...
	err := blockchain.InsertIntoMainChainSyncTxQueue(name, "TxSync", uint32(SyncTxSize), time.Now(), int(bz.MCRoundNumber.Load()), totalPoR)
...
func (bz *ChainBoost) StoragePaymentMainChainBCTransactionQueueCollect(name string) error {
	...
	for i := 0; i < len(mcFirstQueueTxs); i += 2000 {
		...
		err = blockchain.BulkInsertIntoMainChainFirstQueue(name, mcFirstQueueTxs[i:int(limit)])
