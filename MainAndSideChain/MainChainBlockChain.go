...
func (bz *ChainBoost) syncMainChainBCTransactionQueueCollect(name string) (blocksize int) {
	...
	var SummTxDisp int
	for i, v := range bz.SCTools[name].SummDisputes {
		totalPoR = totalPoR + v
		if v != 0 {
			SummTxDisp++ // counting the number of active contracts to measure the length of summary tx
		}
		bz.SCTools[name].SummDisputes[i] = 0
	}
	...
	SummTxsSizeInSummBlock += blockchain.SCSummaryDisputeTxMeasurement(SummTxDisp)
